...
if st.session_state.authentication_status:
    ...
    for i, num in enumerate(pf_df.loc[st.session_state.username]):
        ...
        temp = col2.number_input(pf_df.columns[i], 0.0, 1.0, float(num), 0.05, key=pf_df.columns[i], label_visibility='collapsed')

    if st.session_state.username in pf_df.index:
        st.plotly_chart(px.pie(pf_df, values=st.session_state.username, hole=0.4))
    else:
        st.write('not found')
